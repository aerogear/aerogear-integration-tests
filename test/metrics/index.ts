...
const expectedData = {
    ...
    device: {
        platform: appiumOpts.capabilities.device.includes("iPhone")
            ? "ios"
            : "android",
...
describe("App Metrics", function() {
    ...
    it("should receive app metrics", async () => {
        ...
        result.rows[0].event_type.should.be
            .a("string")
            .and.satisfy(v => v.startsWith("init"));
        result.rows[0].data.device.platformVersion.should.be
            .a("string")
            .and.satisfy(
                v =>
                    v.startsWith(appiumOpts.capabilities.os_version) ||
                    appiumOpts.capabilities.os_version.startsWith(v)
            );
        result.rows[0].data.device.platformVersion =
            appiumOpts.capabilities.os_version;
